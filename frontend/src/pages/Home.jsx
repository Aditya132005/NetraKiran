...
import { useInView, useCounter } from '../utils/useInView'
  ...
  return (
    <div>
      ...
      <section className="relative bg-gradient-to-br from-navy-900 via-navy-800 to-navy-900 text-white overflow-hidden">
        <div className="absolute inset-0 opacity-10" style={{backgroundImage:"url('https://images.unsplash.com/photo-1509695507497-903c140c43b0?w=1600&q=60')", backgroundSize:'cover', backgroundPosition:'center'}}/>
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-20 sm:py-32">
          <div className="max-w-2xl">
            ...
            <h1 className="font-heading text-4xl sm:text-5xl lg:text-6xl font-bold mb-4 leading-tight animate-fade-in-up" style={{animationDelay:'0.25s'}}>
              Netra Kiran<br/>
              <span className="text-gold-400">Optics</span>
            </h1>
            <p className="text-navy-200 text-lg sm:text-xl mb-3 leading-relaxed animate-fade-in-up" style={{animationDelay:'0.4s'}}>
              Premium eyewear &amp; expert optical services in Indirapuram, Ghaziabad.
            </p>
            <p className="text-navy-300 text-sm mb-8 flex items-start gap-1.5 animate-fade-in-up" style={{animationDelay:'0.5s'}}>
            ...
            <div className="flex flex-wrap gap-3 animate-fade-in-up" style={{animationDelay:'0.65s'}}>
      ...
      <section className="bg-gold-600 text-white" ref={statsRef}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex flex-wrap justify-center sm:justify-between gap-6 text-center">
            <div className={statsInView ? 'animate-fade-in-up' : 'opacity-0'} style={{animationDelay:'0s'}}>
              <div className="font-heading font-bold text-2xl">{statsInView ? `${years}+` : '0+'}</div>
              <div className="text-amber-100 text-xs">Years of Trust</div>
            </div>
            <div className={statsInView ? 'animate-fade-in-up' : 'opacity-0'} style={{animationDelay:'0.15s'}}>
              <div className="font-heading font-bold text-2xl">{statsInView ? `${customers.toLocaleString()}+` : '0+'}</div>
              <div className="text-amber-100 text-xs">Happy Customers</div>
            </div>
            <div className={statsInView ? 'animate-fade-in-up' : 'opacity-0'} style={{animationDelay:'0.3s'}}>
              <div className="font-heading font-bold text-2xl">{statsInView ? `${brands}+` : '0+'}</div>
              <div className="text-amber-100 text-xs">Premium Brands</div>
            </div>
            <div className={statsInView ? 'animate-fade-in-up' : 'opacity-0'} style={{animationDelay:'0.45s'}}>
              <div className="font-heading font-bold text-2xl">Free</div>
              <div className="text-amber-100 text-xs">Home Delivery Available</div>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6" ref={catsRef}>
          {[
            ...
            <Link key={cat} to={`/shop?category=${cat}`}
              className={`card border-2 ${color} p-8 text-center hover:scale-105 transition-transform duration-200 ${catsInView ? 'animate-fade-in-up' : 'opacity-0'}`}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 text-center" ref={whyRef}>
          {[
            ...
            <div key={title} className={`card p-6 ${whyInView ? 'animate-fade-in-up' : 'opacity-0'}`} style={{animationDelay:`${i * 0.12}s`}}>
